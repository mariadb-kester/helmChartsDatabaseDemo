...
buildMasterReplicaDemoUK:
	...
	helm install mariadb mariadb-kester-repo/masterreplica -n uk
...
buildBasicGaleraClusterUK:
	...
	helm install ukdc mariadb-kester-repo/galera -n uk \
...
buildBasicGaleraClusterUS:
	...
	helm install usdc mariadb-kester-repo/galera -n us \
...
buildBasicGaleraClusterEU:
	...
	helm install usdc mariadb-kester-repo/galera -n eu \
...
buildGaleraClusterUSCloneFromUK:
	...
	helm install usdc mariadb-kester-repo/galera \
	  --set galera.domainId=200 \
		--set galera.autoIncrementOffset=4 \
		--namespace=us \
		--set cloneRemote=ukdc-galera-backupstream.uk.svc.cluster.local  \
		--set remoteMaxscale=ukdc-galera-masteronly.uk.svc.cluster.local
...
buildGaleraClusterUKCloneFromUS:
	...
	@echo "... and establishing Replicaton"
	helm install ukdc mariadb-kester-repo/galera \
	  --set galera.domainId=100 \
		--set galera.autoIncrementOffset=1 \
		--namespace=uk \
		--set cloneRemote=usdc-galera-backupstream.us.svc.cluster.local  \
		--set remoteMaxscale=usdc-galera-masteronly.us.svc.cluster.local
...
buildGaleraClusterUKCloneFromUSWithMaxscaleReplication:
	...
	helm install ukdc mariadb-kester-repo/galera \
	  --set galera.domainId=100 \
		...
		--set maxscale.changeMaster.host1=usdc-galera-masteronly.us.svc.cluster.local
...
buildGaleraClusterUKWithMaxscaleReplicationFromUSandEU:
	...
	helm install ukdc mariadb-kester-repo/galera \
	  --set galera.domainId=100 \
		...
		--set maxscale.changeMaster.host2=eudc-galera-masteronly.eu.svc.cluster.local
...
buildGaleraClusterUSCloneFromUKWithMaxscaleReplicationFromUKandEU:
	...
	helm install usdc mariadb-kester-repo/galera \
	  --set galera.domainId=200 \
		...
		--set maxscale.changeMaster.host1=ukdc-galera-masteronly.uk.svc.cluster.local \
...
buildGaleraClusterEUCloneFromUSWithMaxscaleReplicationFromUSandUK:
	...
	@echo "... and configuring replication from the US"
	@echo "... and configuring replication from the UK"
	helm install eudc mariadb-kester-repo/galera \
	  --set galera.domainId=300 \
		--set galera.autoIncrementOffset=7 \
		--namespace=eu \
		--set cloneRemote=usdc-galera-backupstream.us.svc.cluster.local  \
		...
		--set maxscale.changeMaster.host2=ukdc-galera-masteronly.uk.svc.cluster.local
